...
func (c Checker) Diagnose(ctx context.Context, request Request, lanAddresses []string) Result {
	result := Result{
		...
		Privacy: []string{
			...
			"Xuva does not use a vendor relay for this check.",
		},
	}
	...
	if err := c.dial(checkCtx, "tcp", address); err != nil {
		...
		result.Summary = "Xuva could not open a TCP connection to the remote address."
...
func Guidance(class string) []string {
	...
	case ClassPrivateRoute:
		return []string{"Use this address only on LAN, VPN, or a mesh network.", "For public internet access, configure a reverse proxy or port forward with HTTPS.", "Do not expose Xuva directly without authentication and TLS."}
	case ClassDNS:
	...
	case ClassNATFirewall:
		return []string{"Forward the chosen external port to the Xuva server or reverse proxy.", "Allow the port through the Windows/Linux firewall.", "If your ISP uses CGNAT, use a VPN/mesh network or reverse proxy tunnel you control."}
