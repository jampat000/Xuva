...
enum VyrdenAPIError: Error, LocalizedError {
    case invalidBaseURL
    ...
    var errorDescription: String? {
        ...
        case .invalidResponse:
            return "Vyrden returned data this app could not read."
        case .server(let status):
            return "Vyrden returned HTTP \(status)."
        }
    }
}

final class VyrdenAPIClient {
    ...
    convenience init(serverURL: String) throws {
        guard let baseURL = URL(string: serverURL.trimmingCharacters(in: .whitespacesAndNewlines)) else {
            throw VyrdenAPIError.invalidBaseURL
        }
    ...
    private func decode<T: Decodable>(data: Data, response: URLResponse) throws -> T {
        guard let http = response as? HTTPURLResponse else {
            throw VyrdenAPIError.invalidResponse
        }
        guard (200..<300).contains(http.statusCode) else {
            throw VyrdenAPIError.server(http.statusCode)
        }
