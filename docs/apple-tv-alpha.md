...
Apple TV is Vyrden's first native playback client target. It should prove that Vyrden is more than a server admin console: a user can install the local server, add media, pair the TV app, browse their library, and play a movie or episode from the couch.
...
- Pairing code flow against the local Vyrden server.
...
1. Start the Vyrden desktop alpha server.
