...
Xuva helps users diagnose remote access without operating a vendor relay. The product checks the route the user owns, classifies the likely failure, and returns concrete next actions.
...
```json
{
  ...
  "route": "public",
  "summary": "Xuva could not open a TCP connection to the remote address.",
...
- No remote diagnostic request uses Xuva-hosted relay infrastructure.
...
2. Xuva returns `nat_firewall`.
3. User forwards the public port to the reverse proxy or Xuva host, opens the OS firewall, or switches to VPN/mesh if CGNAT blocks inbound access.

Certificate failure:

1. TCP connects, TLS fails.
2. Xuva returns `certificate`.
