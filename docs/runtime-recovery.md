...
Vyrden persists operational runtime state in SQLite so a restart does not hide active playback, conversions, media checks, or prepared download jobs.
...
The migration only adds `runtime_entities` and indexes. Rolling back the runtime persistence feature can leave the table in place because older builds ignore it. If a hard rollback is required, stop Vyrden and drop `runtime_entities`; no media library, metadata, or user records depend on it.
