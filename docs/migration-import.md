...
Xuva migration import is designed to lower first-use friction for people moving from Plex, Emby, or Jellyfin without pretending to be a full ecosystem clone.
...
- `schema` must be `xuva.migration.v1`.
...
If a playback row matches a logical movie or episode but Xuva cannot safely pick one local file version, the row stays in conflict until the import bundle is narrowed.
