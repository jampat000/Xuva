...
Lorivo's first alpha should ship as a user-launched desktop app with a tray/taskbar presence, not as a background system service. The server still runs locally, but the desktop shell owns install-time ergonomics: open the web UI, restart the server, expose native folder pickers, and show basic health.

## Folder Selection

The web UI supports a desktop bridge at `window.lorivoDesktop.pickFolder(request)`.
...
- Open Lorivo launches `http://127.0.0.1:8097`.
- Restart Lorivo restarts the supervised server process.
