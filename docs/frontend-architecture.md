...
Vyrden's web UI remains a lightweight server-rendered static app: no framework runtime, no build step, and no client bundle pipeline. The app is split into browser-safe modules that also run under Node tests.
...
```js
const sessions = VyrdenApi.validateArrayPayload(payload, "sessions");
```

Errors should surface through `VyrdenErrors.renderErrorBoundary` for full views or `VyrdenErrors.renderInlineError` for live patches. Avoid raw `error.message` in user-facing UI.
